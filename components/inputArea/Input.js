...
const Input = (props) => {
  ...
  const addTask = (e) => {
    ...
    if (inputValue.trim().length || e.target.button) {
      let newObjTask = { id: idValue, input: inputValue, checked: false };
  ...
  return (
    <>
      <div className="input-area">
        <div className="check-all" onClick={() => checkAll()}>
          {(props.tasksArr.length) ? <>
            <div className={(props.toggleAllValue) ? "chek-symb-first-active" : "chek-symb-first"}  ></div>
            <div className={(props.toggleAllValue) ? "chek-symb-second-active" : "chek-symb-second"} ></div></> : null}
        </div>
        <input className="task-create-input" onBlur={(e) => addTask(e)} onKeyPress={(e) => defineKey(e)} onChange={(e) => setInputValue(e.target.value)} value={inputValue} placeholder="What needs to be done?" />
